from apps.gp.controllers.base import GoogleBaseController
...
class GoogleContactsController(GoogleBaseController):
    _credential = None
    _token = None
    ...
    def create_connection(self, connection=None, plug=None, **kwargs):
        ...
        if self._connection_object is not None:
            try:
                ...
                print("Error GoogleContacts attributes {}".format(e))
    ...
    def get_creation_contact_fields(self):
        return (
            'name', 'surname', 'notes', 'email', 'display_name', 'email_home',
            'phone_work', 'phone_home', 'city',
            'address', 'region', 'postal_code', 'country', 'formatted_address')

    def get_display_contact_fields(self):
        return (
            'title', 'notes', 'email', 'displayName', 'email_home',
            'phoneNumber',
            'phone_home', 'city',
            'address', 'region', 'postal_code', 'country', 'formatted_address')
    ...
    def get_contact_list(self,
        ...
        if r.status_code == 200:
            return xml_to_dict(r.text,
                               iterator_string='{http://www.w3.org/2005/Atom}entry')
        return []
    ...
    def _create_contact_xml(self, dictionary):
        if 'email' not in dictionary and 'phone_work' not in dictionary and 'phone_home' not in dictionary:
            raise Exception(
                "Error: es necesario el telefono o el email para crear un contacto.")
        ...
        if 'formatted_address' in dictionary and dictionary[
            'formatted_address']:
    ...
    def create_contact(self, data):
        ...
        url = "https://www.google.com/m8/feeds/contacts/default/full/?oauth_token={0}".format(
            self._token.access_token)
        r = requests.post(url, data=xml_sr,
                          headers={'Content-Type': 'application/atom+xml',
                                   'GData-Version': '3.0'})
        ...
        for item in contact_list:
            ...
            if not q.exists():
                for column in item['content']:
                    if column['tag'] not in ['link', 'id', 'category',
                                             'updated', 'edited']:
                        sd_item = None
                        if column['tag'] in ['email', 'im']:
                            text = re.sub(u"[^\x20-\x7f]+", u"",
                                          column['attrib']['address']).strip() \
                                if column['attrib'][
                                       'address'] is not None else ''
                            sd_item = StoredData(name=column['tag'],
                                                 value=text, object_id=id,
                            ...
                            for column2 in column['content']:
                                ...
                                if column2['tag'] == 'orgName':
                                    sd_item.append(
                                        StoredData(name=column['tag'],
                                                   value=text, object_id=id,
                                                   connection=connection_object.connection,
                                                   plug=plug))
                                else:
                                    sd_item.append(
                                        StoredData(name=column2['tag'],
                                                   value=text, object_id=id,
                                                   connection=connection_object.connection,
                                                   plug=plug))
                        elif column['tag'] in ['extendedProperty']:
                            text = re.sub(u"[^\x20-\x7f]+", u"",
                                          column['attrib']['name']).strip() \
                    ...
                    if new_item:
                        ...
                        self._log.info(
                            'Item ID: %s, Connection: %s, Plug: %s successfully stored.' % (
                                current_id, item.plug.id, item.connection.id),
                            extra=extra)
                except Exception as e:
