...
class GitLabController(BaseController):
    ...
    def create_issue(self, params=None):
        ...
        try:
            ...
            print("RESULTADO CREACION ISSUE", response.json())
        except InvalidGrantError:
            self.refresh_token()
        return response
    ...
    def create_webhook(self):
        ...
        if action == 'Detect Issue Creation':
            ...
            receiving_url = url_base+url_path
            params = (
                ('url', receiving_url),
                ('issues_events', True),
            )
            url = 'https://gitlab.com/api/v4/projects/{0}/hooks'.format(project.value)
            try:
                response = self.do_post(url, params)
            except InvalidGrantError:
                self.refresh_token()
    ...
    def get_target_fields(self):
        return [{'name': 'title', 'type': 'varchar', 'required': True},
                ...
                {'name': 'description', 'type': 'varchar', 'required': True},
                {'name': 'labels', 'type': 'varchar', 'required': False},
                {'name': 'weight', 'type': 'varchar', 'required': False},
                {'name': 'name', 'type': 'varchar', 'required': True},
                {'name': 'notes', 'type': 'varchar', 'required': False},
