...
def update_all_gears():
    ...
    active_gears = Gear.objects.filter(is_active=True,
                                       gear_map__is_active=True).prefetch_related()
    ...
    for gear in active_gears:
        connector = ConnectorEnum.get_connector(
            gear.source.connection.connector_id)
        ...
        print("Assigning plug {0} to queue: {1}.".format(gear.source.id,
...
def update_plug(plug_id, gear_id, **query_params):
    ...
    if acquire_lock():
        print("Updating Plug: {0} from Gear: {1} as {2}".format(
            plug_id, gear_id, plug.plug_type))
        try:
            query_params = {'connection': gear.source.connection,
                            'plug': gear.source, }
            if gear.gear_map.last_sent_stored_data_id is not None:
                query_params['id__gt'] = gear.gear_map.last_sent_stored_data_id
            connector = ConnectorEnum.get_connector(
                plug.connection.connector.id)
            ...
            if plug.plug_type.lower() == 'source':
                ...
                if stored_data.count() > 0:
                    target_connector = ConnectorEnum.get_connector(
                        gear.target.connection.connector_id)
                    update_plug.s(gear.target.id,
                ...
                if stored_data.count() > 0:
                    target_fields = OrderedDict(
                        (data.target_name, data.source_value) for data in
                        GearMapData.objects.filter(gear_map=gear.gear_map))
                    source_data = [{'id': item[0],
                                    'data': {i.name: i.value for i in
                                             stored_data.filter(
                                                 object_id=item[0])}}
                                   for item in stored_data.values_list(
                            'object_id').distinct()]
                    is_first = gear.gear_map.last_sent_stored_data_id is None
                    if is_first:
                        source_data = [source_data[0], ]
                    entries = controller.send_stored_data(source_data,
                                                          target_fields,
                                                          is_first=is_first)
                    ...
                    if entries or connector == ConnectorEnum.MailChimp:
                        ...
                        gear.gear_map.save(update_fields=['last_source_update',
                                                          'last_sent_stored_data_id'])
