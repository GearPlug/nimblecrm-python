...
class ListConnectionView(LoginRequiredMixin, ListView):
    ...
    def get_queryset(self):
        return self.model.objects.filter(user=self.request.user,
                                         connector_id=self.kwargs[
                                             'connector_id']).prefetch_related()
    ...
    def post(self, request, *args, **kwargs):
        ...
        return redirect(
            reverse('plug:create', kwargs={'plug_type': connector_type}))
...
class CreateConnectionView(LoginRequiredMixin, CreateView):
    ...
    def form_valid(self, form, *args, **kwargs):
        ...
        if self.kwargs['connector_id'] is not None:
            c = Connection.objects.create(user=self.request.user,
                                          connector_id=self.kwargs[
                                              'connector_id'])
            ...
            elif connector in [ConnectorEnum.GoogleSpreadSheets,
                               ConnectorEnum.GoogleContacts,
                               ConnectorEnum.GoogleForms,
                               ConnectorEnum.GoogleCalendar,
                               ConnectorEnum.YouTube]:
                form.instance.credentials_json = self.request.session[
                    'google_credentials']
        ...
        return super(CreateConnectionView, self).form_valid(form, *args,
                                                            **kwargs)
    ...
    def get_context_data(self, *args, **kwargs):
        ...
        if connector in [ConnectorEnum.GoogleSpreadSheets,
                         ConnectorEnum.GoogleForms,
                         ConnectorEnum.GoogleContacts,
            ...
            context[
                'authorization_url'] = settings.SLACK_PERMISSIONS_URL + '&redirect_uri={0}{1}'.format(
                settings.CURRENT_HOST, reverse('connection:slack_auth'))
        elif connector == ConnectorEnum.Twitter:
            flow = tweepy.OAuthHandler(settings.TWITTER_CLIENT_ID,
            ...
            self.request.session['oauth_secret_evernote'] = request_token[
                'oauth_token_secret']
            ...
            authorization_url, state = oauth.authorization_url(
                'https://app.asana.com/-/oauth_authorize?response_type=code&client_id={0}&redirect_uri={1}&state=1234'
                    .format(settings.ASANA_CLIENT_ID,
                            settings.ASANA_REDIRECT_URL))
            ...
            context['authorization_url'] = m.authorization_url(
                redirect_uri=settings.MERCADOLIBRE_REDIRECT_URL)
...
class CreateTokenAuthorizedConnectionView(TemplateView):
    ...
    def get(self, request, **kwargs):
        if 'connection_data' in self.request.session:
            ...
            connector = ConnectorEnum.get_connector(
                name=self.request.session['connector_name'])
...
class MercadoLibreAuthView(View):
    def get(self, request, *args, **kwargs):
        m = MercadolibreClient(client_id=settings.MERCADOLIBRE_CLIENT_ID,
                               client_secret=settings.MERCADOLIBRE_CLIENT_SECRET)
        token = m.exchange_code(code=request.GET.get('code'),
                                redirect_uri=settings.MERCADOLIBRE_REDIRECT_URL)
        m.set_token(token)
        user_me = m.me()
        user_id = user_me['id']
        site_id = 'MLC'
        request.session['connection_data'] = {'token': token, 'site': site_id,
                                              'user_id': user_id}
...
class GoogleAuthView(View):
    def get(self, request, *args, **kwargs):
        ...
        if 'google_connection_type' in request.session and code is not None:
            ...
            credentials = get_flow(settings.GOOGLE_AUTH_CALLBACK_URL,
                                   scope=api.scope).step2_exchange(code)
            request.session['connection_data'] = {
...
class SalesforceAuthView(View):
    def get(self, request, *args, **kwargs):
        ...
        response = requests.post(settings.SALESFORCE_ACCESS_TOKEN_URL,
...
class TwitterAuthView(View):
    def get(self, request, *args, **kwargs):
        ...
        self.request.session['connection_data'] = {'token': flow.access_token,
                                                   'token_secret': flow.access_token_secret}
...
class SurveyMonkeyAuthView(View):
    def get(self, request, *args, **kwargs):
        ...
        data = {"client_secret": settings.SURVEYMONKEY_CLIENT_SECRET,
                "code": auth_code,
        ...
        self.request.session[
            'connector_name'] = ConnectorEnum.SurveyMonkey.name
        return redirect(
            reverse('connection:create_token_authorized_connection'))
...
class HubspotAuthView(View):
    def get(self, request, *args, **kwargs):
        code = request.GET.get('code', '')
        data = {'grant_type': 'authorization_code',
                'client_id': settings.HUBSPOT_CLIENT_ID, 'code': code,
...
class ShopifyAuthView(View):
    def get(self, request, *args, **kwargs):
        ...
        params = {'client_id': settings.SHOPIFY_API_KEY, 'code': code,
                  'client_secret': settings.SHOPIFY_API_KEY_SECRET}
        try:
            response = requests.post(url, params).json()
            self.request.session['connection_data'] = {
                'token': response['access_token'], 'shop_url': shop_url}
        ...
        # TODO: error
        return redirect(
            reverse('connection:shopify_success_create_connection'))
...
class MailchimpAuthView(View):
    def get(self, request, *args, **kwargs):
        ...
        data = {"grant_type": "authorization_code",
                ...
                "redirect_uri": settings.MAILCHIMP_REDIRECT_URL,
                "code": auth_code}
...
class AjaxMercadoLibrePostSiteView(View):
    def post(self, request, *args, **kwargs):
        request.session['mercadolibre_site'] = request.POST.get('site_id',
                                                                None)
...
def get_flow(redirect_to, scope='https://www.googleapis.com/auth/drive'):
    return client.OAuth2WebServerFlow(client_id=settings.GOOGLE_CLIENT_ID,
...
class ConnectionUpdateView(UpdateView):
    ...
    login_url = '/accounts/login/'
    success_url = reverse_lazy('connection:manage')
    ...
    def get(self, request, *args, **kwargs):
        if self.kwargs['pk'] is not None:
            ...
            if connector.connection_type == 'special':
                ...
                self.template_name = 'connection/create_with_auth.html'
        return super(ConnectionUpdateView, self).get(request, *args, **kwargs)
    ...
    def post(self, request, *args, **kwargs):
        if self.kwargs['pk'] is not None:
            ...
            self.model, self.fields = ConnectorEnum.get_connector_data(
                connector)
        return super(ConnectionUpdateView, self).post(request, *args, **kwargs)

    def get_context_data(self, *args, **kwargs):
        connection = self.model.objects.get(connection__id=self.kwargs['pk'])
        connector = ConnectorEnum.get_connector(
            connection.connection.connector.id)
        context = super(ConnectionUpdateView, self).get_context_data(**kwargs)
...
def connection_edit(request):
    ...
    c = Connection.objects.get(pk=gear_id)
    print('CONNECTION >>', c)
    p = Plug.objects.get(pk=c.id)
    print('PLUG >>', p)

    return JsonResponse({"Prueba": 'Prueba'})
