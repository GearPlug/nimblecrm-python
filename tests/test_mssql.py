...
from apps.gp.models import Connection, MSSQLConnection, Plug, Action, StoredData, PlugActionSpecification, \
...
class MssqlControllerTestCases(TestCase):
    """Casos de prueba del controlador MSSQL.

    Variables de entorno:
        TEST_MSSQL_SOURCE_HOST: String: Host del servidor..
        TEST_MSSQL_SOURCE_DATABASE String: Nombre de la base de datos.
        TEST_MSSQL_SOURCE_TABLE: String: Nombre de la tabla.
        TEST_MSSQL_SOURCE_PORT: String: Nùmero de puerto.
        TEST_MSSQL_SOURCE_CONNECTION_USER: String: Usuario.
        TEST_MSSQL_SOURCE_CONNECTION_PASSWORD: String: Contraseña.

        TEST_MSSQL_TARGET_HOST: String: Host del servidor..
        TEST_MSSQL_TARGET_DATABASE String: Nombre de la base de datos.
        TEST_MSSQL_TARGET_TABLE: String: Nombre de la tabla.
        TEST_MSSQL_TARGET_PORT: String: Nùmero de puerto.
        TEST_MSSQL_TARGET_CONNECTION_USER: String: Usuario.
        TEST_MSSQL_TARGET_CONNECTION_PASSWORD: String: Contraseña.
    ...
    def setUpTestData(cls):
        ...
        connection = {
            'user': cls.user,
            'connector_id': ConnectorEnum.MSSQL.value
        }
        ...
        mssql_connection1 = {
            ...
            'host': os.environ.get('TEST_MSSQL_SOURCE_HOST'),
            'database': os.environ.get('TEST_MSSQL_SOURCE_DATABASE'),
            'table': os.environ.get('TEST_MSSQL_SOURCE_TABLE'),
            'port': os.environ.get('TEST_MSSQL_SOURCE_PORT'),
            'connection_user': os.environ.get('TEST_MSSQL_SOURCE_CONNECTION_USER'),
            'connection_password': os.environ.get('TEST_MSSQL_SOURCE_CONNECTION_PASSWORD')
        }
        cls.mssql_connection1 = MSSQLConnection.objects.create(**mssql_connection1)
        ...
        mssql_connection2 = {
            ...
            'host': os.environ.get('TEST_MSSQL_TARGET_HOST'),
            'database': os.environ.get('TEST_MSSQL_TARGET_DATABASE'),
            'table': os.environ.get('TEST_MSSQL_TARGET_TABLE'),
            'port': os.environ.get('TEST_MSSQL_TARGET_PORT'),
            'connection_user': os.environ.get('TEST_MSSQL_TARGET_CONNECTION_USER'),
            'connection_password': os.environ.get('TEST_MSSQL_TARGET_CONNECTION_PASSWORD')
        }

        cls.mssql_connection2 = MSSQLConnection.objects.create(**mssql_connection2)

        action_source = Action.objects.get(connector_id=ConnectorEnum.MSSQL.value, action_type='source', name='get row',
                                           is_active=True)
        ...
        cls.plug_source = Plug.objects.create(**mssql_plug_source)

        action_target = Action.objects.get(connector_id=ConnectorEnum.MSSQL.value, action_type='target',
        ...
        cls.plug_target = Plug.objects.create(**mssql_plug_target)
        ...
        cls.action1 = PlugActionSpecification.objects.create(**action_specification1)
        ...
        cls.action2 = PlugActionSpecification.objects.create(**action_specification2)
    ...
    def setUp(self):
        ...
        """
        # self.client = Client()
        self.source_controller = MSSQLController(self.plug_source.connection.related_connection, self.plug_source)
        self.target_controller = MSSQLController(self.plug_target.connection.related_connection, self.plug_target)

    def test_controller(self):
        """Comprueba los atributos del controlador estén instanciados.

        """
        self.assertIsInstance(self.source_controller._connection_object, MSSQLConnection)
    ...
    def test_select_all(self):
        ...
        result = self.source_controller.select_all(unique=self.action1, order_by=self.action2)
    ...
    def test_insert(self):
        ...
        self.assertRaises(Exception,
                          self.valid_query(sql.replace(os.environ.get('TEST_MSSQL_SOURCE_TABLE'), 'Persons')))
