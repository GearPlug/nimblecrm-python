...
class ActiveCampaignControllerTestCases(TestCase):
    ...
    def setUpTestData(cls):
        ...
        _list_source_actions = ['new contact', 'new subscriber', 'unsubscribed contact', 'new task', 'new deal']
    ...
    def _get_hooks(self, action):
        if action == 'new contact':
            return  {'initiated_by': ['admin'], 'contact[tags]': [''], 'date_time': ['2017-11-27T12:51:37-06:00'],
                    'list': ['0'], 'contact[email]': ['daysy@grplug.com'], 'contact[phone]': [''], 'type': ['subscribe'],
            ...
            return {'initiated_by': ['admin'], 'contact[first_name]': ['Lelia2'], 'orgname': [''], 'contact[phone]': [''], 'list': ['1'],
                    'initiated_from': ['admin'], 'type': ['subscribe'], 'contact[last_name]': ['Rubiano'], 'contact[id]': ['10'],
    ...
    def test_download_source_data(self):
        ...
        for _controller in self._list_source_controllers:
            ...
            count_data = len(data)
            result = _controller['controller'].download_source_data(_controller['plug'].connection.related_connection, _controller['plug'], data=data)
    ...
    def test_download_to_store_data(self):
        ...
        """
        for _controller in self._list_source_controllers:
            data = self._clean_data(self._get_hooks(_controller['action']))
            result = _controller['controller'].download_to_stored_data(_controller['plug'].connection.related_connection, _controller['plug'],
                                                             data=data)
